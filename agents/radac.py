...
from utils.quantile_value import ImplicitQuantileCritic
from agents.radac_diffusion import RADACDiffusionActor, RADACDenoiser
from utils.offline_dataset import OfflineReplayDataset
from utils.risk_measures import WangDistortionSampler, CumulativeProspectWeightSampler, PowerDistortionSampler
from torch.distributions import uniform
from agents.common import ExponentialMovingAverage
...
class RADAC:
    ...
    * Additional techniques:
    ...
    def __init__(self,
        ...
        # ----------------------------
        self.policy_model = RADACDenoiser(
            state_dim=state_dim,
            action_dim=action_dim,
        )
        self.actor = RADACDiffusionActor(
            state_dim=state_dim,
            action_dim=action_dim,
            denoiser=self.policy_model,
        ...
        # ----------------------------
        self.critic1 = ImplicitQuantileCritic(
            dim_state=state_dim,
        ...
        self.critic2 = ImplicitQuantileCritic(
            dim_state=state_dim,
            dim_action=action_dim,
            layers_state=[256],
            layers_action=[256],
        ...
        if risk_distortion == 'cvar':
            ...
            self.distr_taus_risk = WangDistortionSampler()
        elif risk_distortion == 'cpw':
            self.distr_taus_risk = CumulativeProspectWeightSampler()
        elif risk_distortion == 'power':
            self.distr_taus_risk = PowerDistortionSampler()
        else:
            raise ValueError("Invalid risk distortion function specified.")
        self.max_action = max_action
        self.action_dim = action_dim

    def train(self, replay_buffer: OfflineReplayDataset, iterations, batch_size=100, log_writer=None):
