...
from agents.common import ExponentialMovingAverage, SinusoidalTimeEmbedding
...
# ---------------------------------------------------------------------------
class RAFMAC:
    """Risk-aware flow-matching actor-critic for offline continuous control."""
    def __init__(self,
        ...
        # critics ---------------------------------------------------------
        def make_q():
            return ImplicitQuantileCritic(state_dim, action_dim,
    ...
    def _unpack(batch:Tuple):
        """Normalize replay-batch tuples into ``(s, a, ns, r, d)`` form.

        If ``r`` has shape other than ``(B,)`` or ``(B, 1)``, the first
        column is treated as the scalar reward channel.
        """
        if len(batch)==5:
            s,a,ns,r,d = batch
        elif len(batch)==3:
        ...
        # Ensure reward and done tensors have shape (B, 1).
        if r.dim()==1:
            r = r.unsqueeze(1)
        elif r.dim()==2 and r.size(1) != 1:
            # Some replay sources may include auxiliary columns; the first column is the scalar reward.
            r = r[:, :1]
        else:
            # Keep the standard (B, 1) case unchanged.
            pass

        if d.dim()==1:
            d = d.unsqueeze(1)

        # Keep RAFMAC critic targets numerically bounded under stochastic-risk relabeling.
