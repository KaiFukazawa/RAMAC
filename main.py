...
import d4rl
from utils.offline_dataset import OfflineReplayDataset
from utils.training_support import print_banner, EarlyStopping
from utils.run_logger import logger, setup_logger
import h5py
...
def load_hyperparameters(env_name: str, config_path: str):
    """Load environment hyperparameters from a YAML config file.

    Supports both of the following layouts:
    1. A flat mapping from env name -> hyperparameter dict.
    2. A nested mapping with ``defaults`` and ``environments`` keys.
    """
    ...
    for param, default_value in defaults.items():
        hyperparameters.setdefault(param, default_value)
...
def lower_tail_mean(values, alpha=0.1):
    """Return the empirical lower-tail mean over the worst alpha fraction."""
    arr = np.asarray(values, dtype=np.float64)
    if arr.size == 0:
        return float('nan')
    k = max(1, int(np.ceil(float(alpha) * arr.size)))
    return float(np.mean(np.sort(arr)[:k]))

def checkpoint_selection_score(metric, eval_res, eval_norm_res, eval_cvar_res, bc_loss_avg):
    """Map a checkpoint-selection metric to a score where larger is better."""
    if metric == 'eval_return':
        return float(eval_res)
    if metric == 'eval_norm':
        return float(eval_norm_res)
    if metric == 'eval_cvar':
        return float(eval_cvar_res)
    if metric == 'bc_loss':
        return -float(bc_loss_avg)
...
def train_agent(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    if args.risky_dataset_path and os.path.isfile(args.risky_dataset_path):
        if args.risky_dataset_path.endswith('.hdf5'):
            ...
            data_sampler = OfflineReplayDataset(data_dict, device=device)
        ...
        # For example, including { 'observations', 'actions', 'rewards', 'next_observations', 'terminals' }
            data_sampler = OfflineReplayDataset(loaded_data, device=device)
        ...
        data_sampler = OfflineReplayDataset(dataset, device=device)
    ...
    while (training_iters < max_timesteps):
        ...
        if args.eval_risky_env and HAS_RISKY_WRAPPERS:
            ...
            eval_res, eval_res_std, eval_norm_res, eval_norm_res_std, ep_scores = eval_policy(
                agent, args.env_name, args.seed, eval_episodes=args.eval_episodes,
                max_episode_steps=args.max_episode_steps
            )
            violation_counts=0.0

        eval_cvar_res = lower_tail_mean(ep_scores, args.eval_cvar_alpha)
        eval_norm_scores = [env.get_normalized_score(s) for s in ep_scores]
        eval_norm_cvar_res = lower_tail_mean(eval_norm_scores, args.eval_cvar_alpha)
        ...
        logger.record_tabular(f'Eval CVaR{args.eval_cvar_alpha}', eval_cvar_res)
        logger.record_tabular(f'Eval N-CVaR{args.eval_cvar_alpha}', eval_norm_cvar_res)
        logger.dump_tabular()

        # 4.4 Model selection (keep top-k models by evaluation metric by default)
        if args.save_best_model and args.top_k >= 0:
            ...
            def save_best_model_and_logs():
                ...
                if HAS_RISKY_WRAPPERS and args.eval_risky_env:
                    ...
                    eval_meta = {
                        ...
                        "selection_rule": "eval_best" if args.selection_metric != 'bc_loss' else "offline_best",
                        "selection_metric": args.selection_metric,
                        "selection_score": float(selection_score),
                        "selected_epoch": int(curr_epoch),
                        "eval_return": float(eval_res),
                        "eval_return_std": float(eval_res_std),
                        "eval_norm": float(eval_norm_res),
                        "eval_norm_std": float(eval_norm_res_std),
                        "eval_cvar": float(eval_cvar_res),
                        "eval_norm_cvar": float(eval_norm_cvar_res),
                        "eval_cvar_alpha": float(args.eval_cvar_alpha),
            ...
            if len(offline_checkpoints) < actual_k:
                offline_checkpoints.append((selection_score, curr_epoch))
                ...
                worst_score, worst_epoch = offline_checkpoints[0]
                if selection_score > worst_score:
                    offline_checkpoints[0] = (selection_score, curr_epoch)
                    offline_checkpoints.sort(key=lambda x: x[0])
                    # Save if improved
                    save_best_model_and_logs()
...
# ------------------------------------------------
def default_eval_horizon(env_name):
    env_key = env_name.lower()
    if "halfcheetah" in env_key:
        return 200
    if "hopper" in env_key or "walker2d" in env_key:
        return 500
    return None


def eval_policy(policy, env_name, seed, eval_episodes=10, max_episode_steps=None):
    ...
    Returns:
        tuple: (avg_reward, std_reward, avg_norm_score, std_norm_score, episode_returns)
    """
    ...
    return avg_reward, std_reward, avg_norm_score, std_norm_score, scores

# ------------------------------------------------
# eval_policy_risky: Evaluate on risky-wrapped environment + count violations
# ------------------------------------------------
def eval_policy_risky(policy, env_name, seed, eval_episodes, algo_name, dataset_name, max_vel, pro_vel_penal, cost_vel, prob_pose_penal, cost_pose, healthy_angle_range, done_if_exceed_factor, max_episode_steps=None):
    """Evaluate a policy on a risky-wrapped environment and log violations."""
    ...
    base_eval_env = gym.make(env_name)
    eval_env = make_risky_env(
        base_eval_env,
        max_vel=max_vel,
        prob_vel_penal=pro_vel_penal,
        cost_vel=cost_vel,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_episode_steps", default=None, type=int,
                        help="Evaluation horizon. Defaults to 200 for HalfCheetah and 500 for Hopper/Walker2d.")
    parser.add_argument('--use_cvar', action='store_true',
                        default=False,
                        help='Enable IQN-based CVaR measurement when supported by the selected RAMAC agent.')
    ...
    parser.add_argument('--save_best_model', action='store_true',
                        default=False)
    parser.add_argument("--selection_metric", default='eval_return',
                        choices=['eval_norm', 'eval_return', 'eval_cvar', 'bc_loss'],
                        help="Metric used by --save_best_model. Defaults to best raw eval return.")
    parser.add_argument("--eval_cvar_alpha", default=0.1, type=float,
                        help="Lower-tail fraction used to compute evaluation CVaR from rollout returns.")
    ...
    parser.add_argument("--algo", default=os.getenv('ALGORITHM', 'radac'), type=str,
                        choices=['radac', 'rafmac'],
                        help="RAMAC agent to train: 'radac' or 'rafmac'.")
    ...
    if args.max_episode_steps is None:
        args.max_episode_steps = default_eval_horizon(args.env_name)
